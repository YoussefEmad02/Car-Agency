...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM service WHERE Service_Number = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $service_number);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        // Service found, display form to edit its information
...
<body>
    ...
    <section class="edit_service container my-5" id="services">
        <h2>Edit Service with Number <span class="danger"><?php echo $row['Service_Number']; ?></span></h2>
        <form class="row g-3" action="update_service.php" method="post">
            <input type="hidden" name="service_number" value="<?php echo $row['Service_Number']; ?>">
            <div class="col-md-6">
